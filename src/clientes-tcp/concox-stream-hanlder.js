...
const config = {
    host: process.env.CONCOX_HOST,
    port: process.env.CONCOX_PORT,
    exclusive: true,
}
...
function connectEventHandler() {
  ...
  }, 'Conectado al server TCP Concox');
...
function errorEventHandler(e) {
  console.log(`Error al conectar al server TCP Concox ${e.code}`);
  if (!retrying) {
    retrying = true;
  }
...
function closeEventHandler() {
  ...
  console.log('Se cerró el server TCP Concox');
