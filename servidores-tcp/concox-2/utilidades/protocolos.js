...
async function paqueteSecion(data, socket) {
    ...
    vehiculoEncontrado.estado_vehiculo.cambio_motor = null;
    ...
    data_reset.organizacion_id=vehiculoEncontrado.organizacion_id;
                
    //obtener flota id
    data_reset.flota_id=vehiculoEncontrado.flota_id;
                        
    //obtener descripcion vehiculo
    data_reset.description=vehiculoEncontrado.vehiculo_placa;
       
    //obtener protocolo
    data_reset.protocolo='WETRACK-2';
...
async function paqueteLatidos(data, socket) {
    ...
    //estado del motor del vehiculo 
    let estado_motor=contenidoInformativo.estado_motor;
    ...
    let alerta_motor = alertas.generarAlertaMotor(estado_motor, socket.id);
    ...
    if (alerta_motor!=null) {
        ...
        //confirmar cambio de motor
        vehiculo.estado_vehiculo.cambio_motor=true;
        ...
        await db.actualizarEstatusVehiculo(alerta_motor);
    }
    ...
    if (alerta_bateria_vehiculo==null && alerta_motor==null) {
...
async function buscarUbicacion(data,socket, cantidad_sincronizar) {
    ...
    if (!vehiculo.estado_vehiculo.cambio_motor) {   

        //obtener estado gps
        let estado_gps=operacion.obtenerEstadoGps(vehiculo, socket.id);

        //validar que no sea detenido con motor apagado
        if (vehiculo.estado_vehiculo.estado_motor==0 && estado_gps==61715) {
            return null;
        }
        ...
        // si existe un cambio de motor se genera la alerta
        let descripcion = "Vehiculo apagado" ;
        let codigo_gps = "62467" ;

        if (vehiculo.estado_vehiculo.estado_motor==1) {
        ...
        vehiculo.estado_vehiculo.cambio_motor=null
...
async function paqueteUbicacion(trama,socket) {
   ...
   let lista_alertas=[];
    ...
    //insertar en la tabla de eventos de nuevas ubicaciones de zona
...
async function paqueteAlarma(data,socket) {
    ...
    if (buscar_alerta_protocolo_alerta) {
        ...
        //actualizar en la base de datos en estado de vehiculo
        db.actualizarEstatusVehiculo(buscar_alerta_protocolo_alerta.objeto_respuesta);
        // Actualizar estado del vehiculo en la clase
        //tcpSocket.actualizarEstadoVehiculo(socket.id, buscar_alerta_protocolo_alerta.objeto_respuesta);
    ...
    if (buscar_alerta_lenguaje_alerta) {
        ...
        // Actualizar estado del vehiculo en la clase
        //tcpSocket.actualizarEstadoVehiculo(socket.id, buscar_alerta_lenguaje_alerta.objeto_respuesta);
        //insertar evento a la base de datos
...
async function buscarAlertaLenguajeAlerta(data,socket) {
    ...
    let contenido_alerta = operacion.obtenerAlertaLenguajeGps(data, socket);
